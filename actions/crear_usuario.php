...
if ($nombre === '' || $usuario === '' || strlen($password) < 6) {
    header('Location: ' . BASE_URL . '/importar_productos.php?error=Complete todos los datos del usuario');
    exit;
}
...
try {
    ...
    header('Location: ' . BASE_URL . '/importar_productos.php?msg=Usuario creado correctamente');
} catch (PDOException $exception) {
    header('Location: ' . BASE_URL . '/importar_productos.php?error=No se pudo crear el usuario');
}
exit;
